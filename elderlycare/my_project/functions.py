...
def clean(img):
    ...
    # cv2.imshow("jjj",gray)
    (rectval, img_bw) = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)

    print(type(img_bw))
    aaa = cv2.findContours(img_bw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)


    print(aaa,"heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")


    # a,contours, hierarchy = cv2.findContours(img_bw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours, hierarchy = cv2.findContours(img_bw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    print("cntrs",contours)

    # take largest contour
    ctr = sorted(contours, key=lambda ctr: (cv2.boundingRect(ctr)[2] * cv2.boundingRect(ctr)[3]),reverse=True)[0]
    # Get bounding box
...
def process_folder(folder):
    ...
    for img in sorted(os.listdir(folder)):
        if img.endswith(extension):
            try:
                print("ohohohohoh",os.path.join(folder, img))
                image = read_transparent_png(os.path.join(folder, img))
                # cv2.imshow("hhhhh",image)
                new_img = clean(image)
