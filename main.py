...
def process_video(video_path):
    ...
    for image in tqdm(_frames):
        ...
        print(type(_frames[k]))
        k=k+1


        cv2.imshow("dhfkjsdf",_frames[k])
        ...
        if is_fall:
            da= datetime.datetime.now().strftime("%Y%m%d%H%M%F")+".jpg"
            mpath="C:\\Users\\user\\PycharmProjects\\elderlycare\\media\\" +da
            cv2.imwrite(mpath,_frames[k])
            mediapath="/media/"+da
            qry="INSERT INTO `my_project_fall_notification` (`time`,`date`,`photo`,`PATIENT_id`,notification) VALUES (CURTIME(),CURDATE(),'"+mediapath+"','"+str(5)+"','Fall detected')"
            db=Db()
            db.insert(qry)
            print("Is Falling")
        else:
